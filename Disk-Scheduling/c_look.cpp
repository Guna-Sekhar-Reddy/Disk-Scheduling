//disk scheduling using c-look method
...
int main()
{
	...
	if(d<mid)														//if initial cylinder is less(left) than middle one
	{
		...
		for(int i=d;i>=p[min].cno;i--)
		{
			...
			{
				sum= sum+p[max].cno-p[min].cno;									// going to max clinder without checking
	...
	else													//if initial cullinder is more(right) than middle one
	{
		for(int i=d;i<=p[max].cno;i++)
		{
			...
			{
				sum=sum+p[max].cno-p[min].cno;									//coming back to min cylinder without checking
				for(int k=p[min].cno;k<=p[max].cno;k++)
				{
					for(int j=0;j<n;j++)
					{
						if(p[j].flag==0 and p[j].cno==cyl[k])
